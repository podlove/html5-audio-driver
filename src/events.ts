...
import { getNodeFromEvent, collectProperties, toArray } from "./utils";
import {
  props,
  playtime,
  volume,
  ...
  curry((media, callback, runtimeOptions = {}) => {
    toArray(events).forEach((event) => {
      media.addEventListener(
        event,
        compose<[EventTarget], any, any, void>(
          callback,
          processor,
          getNodeFromEvent
        ),
...
const onBuffering = curry(
  (media: MediaElement, callback, runtimeOptions = {}) => {
    media.addEventListener(
      "waiting",
      (event) => {
...
const onError = curry(
  (
    media: MediaElement,
    callback: (error: ErrorState, detail?: any) => void
  ) => {
    media.addEventListener(
      "error",
      function ({ detail }: any) {
